...
function renderStoreQr(walletCode) {
  ...
  if (!walletCode) {
    qrContainer.innerHTML =
      "<p style='color:var(--text-muted);font-weight:700'>No wallet code</p>";
    return;
  }
...
async function loadHistory(range) {
  ...
  try {
    const { res, data } = await apiGet(
      `/wallet/history?range=${encodeURIComponent(range)}`,
    );
...
function resetForgotPinModalState() {
  ...
  const remaining = getRemainingForgotPinCooldown();
  if (remaining > 0) {
    forgotPinCooldownEnd = Number(
      localStorage.getItem("forgotPinCooldownEnd") || 0,
    );
    startForgotPinCooldownTimer();
  } else {
    clearForgotPinCooldown();
...
async function sendForgotPinCode() {
  ...
  try {
    const { res, data } = await apiPost("/wallet/forgot-pin/request", {
      email,
    });
