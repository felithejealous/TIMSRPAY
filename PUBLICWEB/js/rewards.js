...
function normalizeTransaction(tx) {
    const rawType = String(tx.type || tx.transaction_type || "earn").toLowerCase();
...
function renderHistory(list, target, limit = null) {
    ...
    target.innerHTML = items.map(raw => {
        ...
        const subtitle = tx.displayOrderId ? `${formatDateTime(tx.date)} • ${tx.displayOrderId}` : formatDateTime(tx.date);
...
function renderRewards(points, rewards = null) {
    ...
    const rewardRows = Array.isArray(rewards) && rewards.length
        ? rewards
        : [
            {
                reward_id: 1,
    ...
    rewardsList.innerHTML = rewardRows.map(reward => {
        const requiredPoints = Number(reward.points_required || 2800);
        const canClaim = typeof reward.claimable === "boolean"
            ? reward.claimable
...
async function handleRedeem(button) {
    try {
        ...
        if (redeemExpiry) redeemExpiry.textContent = formatExpiry(expiresAt, hasExpiry);

        if (window.QRCode && redeemQrCanvas) {
            redeemQrCanvas.getContext("2d").clearRect(0, 0, redeemQrCanvas.width, redeemQrCanvas.height);
