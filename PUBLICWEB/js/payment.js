...
function updateQty(index, delta) {
    ...
    checkoutItems[index].total = Number((unitPrice * checkoutItems[index].qty).toFixed(2));
...
function selectPayment(element, type) {
    ...
    if (type === "wallet") {
        ...
        if (checkoutNote) {
            checkoutNote.textContent = "Wallet payment will charge your TeoPay balance immediately.";
        }
    } else {
        rpayForm?.classList.remove("active");
        cashForm?.classList.add("active");
        if (checkoutNote) {
            checkoutNote.textContent = "Cash payment will create a pending online order to be paid at pickup.";
        }
    }
}
...
function buildOrderPayload() {
    return {
        ...
        wallet_email: currentPayment === "wallet"
            ? (document.getElementById("rpayEmail")?.value || "").trim()
            : null,
        wallet_pin: currentPayment === "wallet"
...
function validateBeforeSubmit(payload) {
    ...
    if (payload.payment_method === "wallet") {
        ...
        if (!payload.wallet_pin || payload.wallet_pin.length < 4 || payload.wallet_pin.length > 6) {
...
function showSuccessReceipt(orderData) {
    ...
    if (printOrderNum) {
        printOrderNum.textContent = orderData?.display_id || `#${orderData?.order_id || "—"}`;
    }

    if (printItemsList) {
        printItemsList.innerHTML = checkoutItems.map(item => `
    ...
    if (printMethod) printMethod.textContent = paymentMethod === "wallet" ? "Teo Pay" : "Cash";
    ...
    if (printFooterNote) {
        printFooterNote.textContent = paymentMethod === "wallet"
