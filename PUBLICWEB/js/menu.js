...
function getProductImage(product) {
    ...
    const fallback = FALLBACK_PRODUCT_IMAGES[(product.name || "").trim().toLowerCase()];
    return fallback || "mango.png";
}
...
function getSelectedAddOns() {
    return Array.from(document.querySelectorAll(".addon-check:checked")).map(input => ({
...
function syncCheckoutDraft() {
    const payload = {
        ...
        subtotal_preview: Number(cart.reduce((sum, item) => sum + Number(item.total || 0), 0).toFixed(2))
    };
...
function updateModalPrice() {
    ...
    const unitPrice = Number(currentProduct.price || 0) + Number(sizeMeta.addPrice || 0) + addOnsTotal;
...
async function initMenuPage() {
    // Wait for auth guard first before fetching menu
    const isAuthed = await checkCustomerAuth();
    if (!isAuthed) return; // stop if not authenticated
