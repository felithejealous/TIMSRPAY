...
def _get_role(db: Session, role_name: str) -> Role:
    ...
    if not r:
        raise HTTPException(status_code=400, detail=f"role '{role_name}' not found in roles table")
    return r
...
def list_staff(
    ...
    # join via manual query (since no relationship used)
    ...
    for u in users:
        sp = sp_map.get(u.id)
        if not sp:
            continue  # only show staff profiles
