...
def list_addon_recipe(
    add_on_id: int,
    db: Session = Depends(get_db),
    _: User = Depends(require_roles("staff", "cashier", "admin")),
):
    rows = (
        db.query(AddOnRecipe, InventoryMaster)
        .join(InventoryMaster, InventoryMaster.id == AddOnRecipe.inventory_master_id)
        .filter(AddOnRecipe.add_on_id == add_on_id)
        .all()
    )
...
@router.post("/addons")
def add_addon_recipe(
    payload: AddOnRecipeCreate,
    db: Session = Depends(get_db),
    _: User = Depends(require_roles("admin")),
):
